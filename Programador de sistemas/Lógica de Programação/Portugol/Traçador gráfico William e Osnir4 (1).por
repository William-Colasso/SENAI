programa
{
    ...
    inteiro CINZA2 = g.criar_cor(128, 128, 128), a = 1, b =0, c=0
	...
	funcao parabola2(){
			se(a>0 e delta>0){
				para(inteiro i = 0; i<99; i+=1){
					...
					g.desenhar_elipse((-vetor_x[i])+750+(xv*zoom), (-vetor_y[i])+335+(yv*zoom),8,8,verdadeiro)
					g.desenhar_elipse((vetor_x[i])+750+(xv*zoom), (-vetor_y[i])+335+i+(yv*zoom),8,8,verdadeiro)
					g.desenhar_elipse((-vetor_x[i])+750+(xv*zoom), (-vetor_y[i])+335-i+(yv*zoom),8,8,verdadeiro)
				}
			}
			se(a<0 e delta>0){
 ...
 * 
 * @POSICAO-CURSOR = 10127; 
 * @PONTOS-DE-PARADA = ;
 * @SIMBOLOS-INSPECIONADOS = {c, 10, 62, 1}-{x, 11, 34, 1}-{xv, 14, 25, 2}-{delta, 14, 41, 5};
