programa
{
    ...
    inteiro CINZA2 = g.criar_cor(128, 128, 128), a = 1, b =-9, c=17
    inteiro cont = 5, cont2 = -5
    inteiro vetor_x[1000], vetor_y[1000], zoom = 15, j = 0, i = 0, n =0,auxi = 0
    ...
    funcao grafico(){
        ...
        se(delta<=0){
			...
			g.definir_cor(g.COR_PRETO)
			g.desenhar_texto(50, 400, "Para delta negativo, existem apenas raízem complexas. ")
	...
	funcao parabola2(){
		para(i=0;i<100;i++){
			g.definir_cor(g.COR_PRETO)
			g.desenhar_linha((vetor_x[i]+750)+(xv*zoom), (-vetor_y[i]+335)+(yv*zoom), (vetor_x[i]+750)+(xv*zoom), (-vetor_y[i+1]+335)+(yv*zoom))
 ...
 * 
 * @POSICAO-CURSOR = 9461; 
 * @DOBRAMENTO-CODIGO = [27, 79];
 * @PONTOS-DE-PARADA = ;
 * @SIMBOLOS-INSPECIONADOS = {vetor_x, 12, 12, 7}-{vetor_y, 12, 27, 7};
